...
Para realizar este estudio se empieza mostrando las herramientas matemáticas necesarias para desarrollar algunos de los esquemas fundamentales de clave pública, esto se verá en el capítulo \ref{chap:herramientas}.\\
Posteriormente se trata en más detalle las curvas elípticas en el capítulo\ref{chap:EC}, que se usarán junto con las herramientas anteriores. En este apartado se hace uso del código proporcionado en el anexo para generar la figura que representa la operación fundamental que se usa en las curvas elípticas.\\
Una vez presentados los conocimientos básicos con los que se trabaja, se pasa a mostrar los esquemas de clave pública así como algún ejemplo de los mismos en el capítulo\ref{chap:public key}.\\
Finalmente en el capítulo \ref{chap:identidad} se usa la mayoría de conceptos anteriores para plantear un esquema de clave pública sobre curvas elípticas, el cifrado basado en identidad.\\
Tras ello se encuentra el apartado de conclusiones donde muestro algunas de las ideas generales que me ha trasmitido este estudio y que he aprendido al realizarlo.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% OBJETIVOS %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Sea $G$ un grupo cíclico finito generado por $g$, puesto que $G$ es finito se tiene que $g^n=e$ para algún $n>0$. Asumimos que $n$ sea el menor posible (el orden de $g$).\\
Se puede escribir $G$ como $\left\lbrace e, g, ..., g^{n-1} \right\rbrace$ donde todos los elementos son distintos, es decir, que si $m_1,m_2 \in \left\lbrace 0, 1, ..., n-1 \right\rbrace$ tal que $m_1>m_2$, entonces $g^{m_1}\not=g^{m_2}$, de no ser así se tendría que $g^{m_1-m_2}=e$ que contradice que $n$ sea el orden de $g$.\\
Usando el algoritmo de la división se puede escribir cualquier entero $m$ como $m=nq+r$, donde $r=m\ mod\ n$. Por tanto $g^m=(g^n)^qg^r=g^r=g^{m\ mod\ n}$. Definimos $f: \mathbb{Z}_n \rightarrow G$ donde $f(m)=g^m$, esta función corresponde cada elemento uno a uno. Finalmente se puede comprobar que además es un isomorfismo ya que
\[
f(m_1)f(m_2)=g^{m_1}g^{m_2}=g^{m_1+m_2}=g^{(m_1+m_2)\ mod\ n}=f(m_1\oplus m_2)
\]
...
\[
\mathbb{P}^2(K) = \dfrac{K^3\backslash(0,0,0)}{Eq}
\]
...
\end{itemize}
\end{itemize}
\underline{Teorema}: El cifrado RSA presentado es correcto.\\
...
\]
\underline{Teorema}: El cifrado basado en identidad presentado es correcto.\\
\underline{Demostración}: Corrección del esquema.\\
